...
STATE_FILE = "/opt/airflow/dags/state.json"
...
def parse_date(created_at):
    ...
    if re.match(r'\d{2}:\d{2}', created_at) or re.match(r'\d+시간 전', created_at):
        return f"{YEAR}-{datetime.today().strftime('%m-%d')}"
...
def crawl_quasarzone_category(category, base_url):
    ...
    last_date_str = state.get(category, "2022-01-01")
    last_collected = datetime.strptime(last_date_str, "%Y-%m-%d")
    ...
    df = df[df['created_at'] > last_date_str]
...
with DAG(
    ...
    [task_pc_hardware, task_notebook_mobile] >> glue_transform
