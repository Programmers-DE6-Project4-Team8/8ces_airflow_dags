...
# 크롤링 함수
def scrape_pc_data():
    base_url = "https://quasarzone.com/bbs/qb_saleinfo?_method=post&_token=WFvgK7Uo3vRnLEmg2qUVskvNlgih0KNrTrGKiRaV&category=PC%2F%ED%95%98%EB%93%9C%EC%9B%A8%EC%96%B4&kind=subject&sort=num%2C%20reply&direction=DESC&page={}"
    hotdeal_info = []
    page = 1
    ...
    while True:
        ...
        for row in rows:
            try:
                ...
                if not created_at:
                    print("\ud83d\udec0 2022-01-01 이전 글 발견. 종료")
                    return save_and_upload_pc(hotdeal_info)
    ...
    save_and_upload_pc(hotdeal_info)

# 저장 및 업로드 함수
def save_and_upload_pc(data):
    timestamp = datetime.now().strftime("%Y-%m-%d")
    filename = f"hotdeal_pc_{timestamp}.json"
...
dag = DAG(
    dag_id='hotdeal_pc_scraper_data',
    default_args=default_args,
    schedule_interval='@daily',
    catchup=False,
    tags=['hotdeal', 'pc', 'scraper']
)

scrape_pc_task = PythonOperator(
    task_id='scrape_pc_data_task',
    python_callable=scrape_pc_data,
    dag=dag
)
