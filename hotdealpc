...
upload_to_s3 = LocalFilesystemToS3Operator(
    ...
    filename='/tmp/hotdeal_data.json',  # 로컬에 저장된 파일 경로
    bucket_name='de6-team8-bucket',  # S3 버킷 이름
    key='raw_data/quasarzone/hotdeal_data.json',  # S3 저장 경로
    dest_key='raw_data/quasarzone/hotdeal_data.json',  # S3 저장 경로 (dest_key 추가)
)
...
with DAG(
    ...
    scrape_task >> upload_to_s3  # 스크래핑 후 S3에 업로드
