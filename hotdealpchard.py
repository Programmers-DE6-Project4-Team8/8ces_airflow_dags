...
def scrape_recent_data():
    ...
    # 기존 JSON 파일을 읽어서 incremental update (파일이 존재하면 기존 데이터 추가)
    ...
    # 기존 데이터에 새로운 데이터 추가 (incremental update)
    existing_data.extend(hotdeal_info)

    # 데이터를 Line-delimited JSON 형태로 저장
    ...
    # S3 업로드 함수 호출
    upload_to_s3(filename=file_path, key='test/hotdeal_data.json', bucket_name='de6-team8-bucket')
...
# DAG 정의
with DAG(
    dag_id='hotdeal_scraper_recent_data',  # DAG ID 변경
    ...
    scrape_task = PythonOperator(
        task_id='scrape_recent_data_task',  # task_id 변경
